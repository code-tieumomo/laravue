...
import { BreadcrumbItem, Role, SharedData, User } from '@/types';
...
const form = useForm({
    ...
    roles: props.user.roles?.map((role: Role) => role.name) || [],
});
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="flex items-center justify-between">
                <div class="space-y-0.5">
                    <h2 class="text-xl font-semibold tracking-tight">Edit User: {{ user.name }}</h2>
                    <p class="text-muted-foreground text-sm">Update the user details and roles</p>
                </div>
                <div>
                    <Button @click="submit">
            ...
            <form class="mt-4 grid grid-cols-4 gap-8" @submit.prevent="submit">
                ...
                <div class="col-span-1">
                    <Label for="password" class="mb-2">New Password (optional)</Label>
                    <Input id="password" v-model="form.password" type="password" placeholder="New Password" autocomplete="off" />
